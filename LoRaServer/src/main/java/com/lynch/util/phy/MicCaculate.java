...
public class MicCaculate {
     ...
     * @return 4 字节的 MIC
     */
    public byte[] MicCaculate(MacPktForm macpktform, int type) {
...
//        base64__.myprintHex(macbyte);
        ...
        switch (type) {
            ...
            case 2:        // UnConfirmed Data Up
                nwkSKey = DatatypeConverter.parseHexBinary(stringRedisTemplate.opsForValue().get(ObjectToString.byteToStrWith(macpktform.getDevAddr()) + "-NwkSKey"));
//                appSKey = DatatypeConverter.parseHexBinary(stringRedisTemplate.opsForValue().get(ObjectToString.byteToStrWith(macpktform.getDevAddr())+ "-AppSKey"));
